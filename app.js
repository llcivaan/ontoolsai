...
const TRADES = [
  ...
  {id:"contractor",label:"General Contractor",emoji:"🏗️"},
];
...
const AFFILIATES={
  ...
  review_request:{name:"NiceJob",cta:"Automate review collection after every job",desc:"NiceJob sends review requests automatically. Average 4x more reviews.",link:"https://try.nicejob.com/ixkns2ovomv8"},
  positive_review:{name:"NiceJob",cta:"Monitor every review across every platform",desc:"See all your Google, Yelp & Facebook reviews in one place, instantly.",link:"https://try.nicejob.com/ixkns2ovomv8"},
  ...
  neutral_review:{name:"NiceJob",cta:"Turn average customers into raving fans — NiceJob",desc:"Automated follow-ups turn 3-star experiences into 5-star reviews.",link:"https://try.nicejob.com/ixkns2ovomv8"},
...
const TRADE_PH = {
  ...
  contractor: {
    customer_name:"e.g. Robert", business_name:"e.g. BuildRight Contracting", your_name:"e.g. Steve, BuildRight",
    job_type:"e.g. kitchen renovation, basement finish", service_area:"e.g. Toronto, ON", speciality:"e.g. full home renovations, additions",
    service:"e.g. kitchen remodel, deck build", offer:"e.g. Free on-site estimate this week", focus:"e.g. spring renovations, hiring",
    role:"e.g. general contractor, site supervisor", location:"e.g. Toronto, ON",
  },
};
...
const DEMO_JOBS={
  ...
  contractor:{customer:"Robert",jobType:"Kitchen renovation",time:"morning",amount:"$12,000",status:"quoted"},
};
...
function OnToolsAI(){
  ...
  const [bonusEmail,setBonusEmail]=React.useState("");
  const [bonusSubmitted,setBonusSubmitted]=React.useState(()=>!!localStorage.getItem("ontoolsai_bonus_claimed"));
  const [bonusError,setBonusError]=React.useState("");
  ...
  React.useEffect(()=>{
    ...
    setJobs(loadJobs());
    // ─── URL param: ?trade=hvac pre-selects trade and skips to module screen ───
    const params=new URLSearchParams(window.location.search);
    const paramTrade=params.get("trade");
    if(paramTrade&&TRADES.find(t=>t.id===paramTrade)){
      setTrade(paramTrade);
      setStep("module");
    }
  },[]);
  ...
  const bonusGranted=bonusSubmitted?5:0;
  const effectiveLimit=FREE_LIMIT+bonusGranted;
  const usageLeft=effectiveLimit-usage;
  ...
  const generate=async(overrideSeq)=>{
    ...
    if(usage>=effectiveLimit){setShowUpgrade(true);return;}
  ...
  return(
    React.createElement('div', { style: {minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif",color:C.text,width:"100%",boxSizing:"border-box",overflowX:"hidden"},}
      ...
      , React.createElement('main', { style: {maxWidth:500,margin:"0 auto",padding:"22px 18px 100px"},}
        ...
        , trade&&tab==="jobs"&&step!=="output"&&(
          React.createElement('div', null
            ...
            /* Calendar — week/month toggle + navigation */
            , (()=>{
              const today=dateKey(new Date());
              const selD=new Date(calDate+"T12:00:00");
              if(calView==="month"){
                const yr=selD.getFullYear(), mo=selD.getMonth();
                const moName=selD.toLocaleDateString("en-US",{month:"long",year:"numeric"});
                ...
                return React.createElement('div', {style:{marginBottom:16}},
                  React.createElement('div', {style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}},
                    React.createElement('button', {onClick:()=>{const p=new Date(yr,mo-1,1);setCalDate(dateKey(p));}, style:{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 8px"}}, "‹"),
                    React.createElement('div', {style:{fontSize:15,fontWeight:800,color:C.text}}, moName),
                    React.createElement('button', {onClick:()=>{const n=new Date(yr,mo+1,1);setCalDate(dateKey(n));}, style:{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 8px"}}, "›")
                  ),
                  React.createElement('div', {style:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}},
                    ["S","M","T","W","T","F","S"].map((d,i)=>React.createElement('div', {key:i, style:{textAlign:"center",fontSize:10,color:C.subtle,fontWeight:600,padding:"4px 0"}}, d))
                  ),
                  React.createElement('div', {style:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}},
                    cells.map((day,i)=>{
                      ...
                      const isT=dk===today;
                      const hasJ=jobs.some(j=>j.date===dk&&(j.trade===trade||!j.trade));
                      return React.createElement('button', {key:dk, onClick:()=>setCalDate(dk),
                        style:{padding:"6px 0",background:isSel?C.amberDim:"transparent",border:isSel?`1px solid ${C.amber}`:"1px solid transparent",borderRadius:8,cursor:"pointer",textAlign:"center",transition:"all 0.1s"}},
                        React.createElement('div', {style:{fontSize:13,fontWeight:isT||isSel?800:500,color:isSel?C.amber:isT?"#FFF":C.muted}}, day),
                        hasJ&&React.createElement('div', {style:{width:4,height:4,background:C.amber,borderRadius:2,margin:"2px auto 0"}})
                      );
                    })
                  ),
                  React.createElement('div', {style:{display:"flex",justifyContent:"center",gap:8,marginTop:10}},
                    React.createElement('button', {onClick:()=>setCalView("week"), style:{background:C.surface2,border:`0.5px solid ${C.border2}`,borderRadius:8,padding:"5px 12px",fontSize:11,color:C.muted,cursor:"pointer",fontWeight:600}}, "Week"),
                    React.createElement('button', {onClick:()=>{}, style:{background:C.amberDim,border:`0.5px solid ${C.amber}`,borderRadius:8,padding:"5px 12px",fontSize:11,color:C.amber,cursor:"pointer",fontWeight:700}}, "Month"),
                    today!==calDate&&React.createElement('button', {onClick:()=>setCalDate(today), style:{background:"none",border:"none",fontSize:11,color:C.amber,cursor:"pointer",fontWeight:600,padding:"5px 8px"}}, "Today ↩")
                  )
                );
              } else {
                return React.createElement('div', {style:{marginBottom:16}},
                  React.createElement('div', {style:{display:"flex",gap:4,alignItems:"center"}},
                    React.createElement('button', {onClick:()=>{const d=new Date(calDate+"T12:00:00");d.setDate(d.getDate()-7);setCalDate(dateKey(d));}, style:{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 6px"}}, "‹"),
                    React.createElement('div', {style:{display:"flex",gap:4,flex:1}},
                      Array.from({length:7},(_,i)=>{
                        const d=new Date(calDate+"T12:00:00");d.setDate(d.getDate()-3+i);
                        const dk=dateKey(d);
                        const isT=dk===today;
                        const isSel=dk===calDate;
                        const hasJ=jobs.some(j=>j.date===dk&&(j.trade===trade||!j.trade));
                        ...
                        return React.createElement('button', {key:dk, onClick:()=>setCalDate(dk),
                          style:{flex:1,minWidth:38,padding:"8px 2px",background:isSel?C.amberDim:C.surface,border:`0.5px solid ${isSel?C.amber:C.border}`,borderRadius:10,cursor:"pointer",textAlign:"center",transition:"all 0.15s"}},
                          React.createElement('div', {style:{fontSize:10,color:isSel?C.amber:C.subtle,fontWeight:600}}, dayLabel),
                          React.createElement('div', {style:{fontSize:15,fontWeight:800,color:isSel?C.amber:isT?"#FFF":C.muted,marginTop:2}}, d.getDate()),
                          hasJ&&React.createElement('div', {style:{width:5,height:5,background:C.amber,borderRadius:3,margin:"3px auto 0"}})
                        );
                      })
                    ),
                    React.createElement('button', {onClick:()=>{const d=new Date(calDate+"T12:00:00");d.setDate(d.getDate()+7);setCalDate(dateKey(d));}, style:{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 6px"}}, "›")
                  ...
                  React.createElement('div', {style:{display:"flex",justifyContent:"center",gap:8,marginTop:10}},
                    React.createElement('button', {onClick:()=>{}, style:{background:C.amberDim,border:`0.5px solid ${C.amber}`,borderRadius:8,padding:"5px 12px",fontSize:11,color:C.amber,cursor:"pointer",fontWeight:700}}, "Week"),
                    React.createElement('button', {onClick:()=>setCalView("month"), style:{background:C.surface2,border:`0.5px solid ${C.border2}`,borderRadius:8,padding:"5px 12px",fontSize:11,color:C.muted,cursor:"pointer",fontWeight:600}}, "Month"),
                    today!==calDate&&React.createElement('button', {onClick:()=>setCalDate(today), style:{background:"none",border:"none",fontSize:11,color:C.amber,cursor:"pointer",fontWeight:600,padding:"5px 8px"}}, "Today ↩")
                  )
                );
              }
...
Reply directly to them (use "you/your"). No bullet points. No "Thank you for your feedback." energy. Real talk.`;
        ...
        , (!trade||(tab==="write"&&step!=="output"))||(step==="output")?(
          React.createElement(React.Fragment, null
            ...
            , step==="module"&&(
              React.createElement('div', null
                , React.createElement('button', { onClick: ()=>setStep("trade"), style: {background:"none",border:"none",color:C.amber,cursor:"pointer",fontSize:13,marginBottom:16,padding:0,fontWeight:600},}, _optionalChain([tradeObj, 'optionalAccess', _77a => _77a.emoji]), " " , _optionalChain([tradeObj, 'optionalAccess', _77b => _77b.label]), " — change trade" )
                , React.createElement('div', { style: {marginBottom:20},}
            ...
            , step==="fields"&&toolObj&&(
              React.createElement('div', null
                ...
                /* Sequence Toggle */
                , toolObj.seq&&(
                  React.createElement('div', { onClick: ()=>{if(usage>=effectiveLimit){setShowUpgrade(true);return;}setIsSeq(!isSeq);},
                ...
                , React.createElement('div', { style: {textAlign:"center",marginTop:8,color:C.subtle,fontSize:11},}
                  , usageLeft, " of "  , effectiveLimit, " free messages remaining this month"
                )
              )
            )
      ...
      , showUpgrade&&(
        React.createElement('div', { style: {position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200,padding:"0 16px 20px"}, onClick: ()=>setShowUpgrade(false),}
          , React.createElement('div', { style: {...card(),padding:22,maxWidth:440,width:"100%",border:`0.5px solid ${C.border2}`}, onClick: e=>e.stopPropagation(),}
            ...
            /* Bonus email capture */
            , !bonusSubmitted&&(
              React.createElement('div', { style: {...card(),padding:"14px 16px",marginBottom:8,background:C.surface2,border:`0.5px solid ${C.border2}`},}
                , React.createElement('div', { style: {fontSize:13,fontWeight:700,marginBottom:4},}, "🎁 Not ready to upgrade? Get 5 bonus messages free"  )
                , React.createElement('div', { style: {fontSize:11,color:C.muted,marginBottom:10},}, "Drop your email — we'll send it straight through, no spam."  )
                , React.createElement('div', { style: {display:"flex",gap:8},}
                  , React.createElement('input', {
                    type:"email",
                    placeholder:"your@email.com",
                    value:bonusEmail,
                    onChange:e=>{setBonusEmail(e.target.value);setBonusError("");},
                    style:{flex:1,background:C.surface3,border:`0.5px solid ${bonusError?C.red:C.border2}`,borderRadius:9,padding:"9px 12px",color:C.text,fontSize:13,outline:"none",fontFamily:"inherit"},
                    onFocus:e=>{e.target.style.borderColor=C.amber;},
                    onBlur:e=>{e.target.style.borderColor=bonusError?C.red:C.border2;},
                  })
                  , React.createElement('button', {
                    onClick:async()=>{
                      const em=bonusEmail.trim();
                      if(!em||!em.includes("@")){setBonusError("Enter a valid email");return;}
                      localStorage.setItem("ontoolsai_bonus_claimed","1");
                      localStorage.setItem("ontoolsai_bonus_email",em);
                      ...
                      try{
                        await fetch("/.netlify/functions/klaviyo-subscribe",{
                          ...
                          body:JSON.stringify({email:em,trade:trade||"unknown"})
                        });
                      }catch(e){console.log("Klaviyo subscribe error:",e);}
                    },
                    style:{background:C.amber,border:"none",borderRadius:9,padding:"9px 14px",color:"#000",fontWeight:900,fontSize:13,cursor:"pointer",whiteSpace:"nowrap"},
                  }, "Claim →")
                )
                , bonusError&&React.createElement('div', {style:{fontSize:11,color:C.red,marginTop:5}}, bonusError)
              )
            )
            , bonusSubmitted&&(
              React.createElement('div', { style: {...card(),padding:"12px 16px",marginBottom:8,background:C.greenDim,border:`0.5px solid ${C.green}40`},}
                , React.createElement('div', { style: {fontSize:13,fontWeight:700,color:C.green}}, "✅ 5 bonus messages added!")
                , React.createElement('div', { style: {fontSize:11,color:C.muted,marginTop:3}}, "Ready to use — enjoy.")
              )
            )
            ...
            , React.createElement('div', { style: {...card(),padding:14,marginBottom:8,border:`1.5px solid ${C.amber}`,position:"relative",background:C.amberDim},}
              ...
              , React.createElement('button', { onClick:()=>window.open("https://ontoolsai.lemonsqueezy.com/checkout/buy/95831c95-f7c4-4e51-98a7-20212b78a902","_blank"), style: {marginTop:12,width:"100%",background:C.amber,border:"none",borderRadius:10,padding:"12px",color:"#000",fontWeight:900,fontSize:14,cursor:"pointer"},}, "Start for $5.99/month →"
