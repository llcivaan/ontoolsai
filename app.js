...
// ─── MAIN APP ──────────────────────────────────────────────────────────────────
 function OnToolsAI(){
  ...
  const generate=async(overrideSeq)=>{
    ...
    if(!isPro&&usage>=FREE_LIMIT){setShowUpgrade(true);return;}
    ...
    React.createElement('div', { style: {minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif",color:C.text},}
      ...
      , React.createElement('header', { style: {padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`0.5px solid ${C.border}`,position:"sticky",top:0,zIndex:100,background:C.bg},}
        ...
        , React.createElement('div', { style: {display:"flex",alignItems:"center",gap:8},}
          ...
          , isPro
            ?React.createElement('div', { style: {fontSize:11,color:C.green,background:C.greenDim,padding:"4px 10px",borderRadius:20,border:`0.5px solid ${C.green}40`},}, isBusiness?"🍳 Business":"☕ Pro")
        ...
        React.createElement('div', { style: {position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200,padding:"0 16px 20px"}, onClick: ()=>setShowUpgrade(false),}
          , React.createElement('div', { style: {...card(),padding:22,maxWidth:440,width:"100%",border:`0.5px solid ${C.border2}`}, onClick: e=>e.stopPropagation(),}
            ...
            , React.createElement('div', { style: {...card(),padding:14,marginBottom:8,border:`1.5px solid ${C.amber}`,position:"relative",background:C.amberDim},}
              ...
              , React.createElement('button', { onClick: ()=>openCheckout("pro"), disabled: checkoutLoading, style: {marginTop:12,width:"100%",background:checkoutLoading?C.surface3:C.amber,border:"none",borderRadius:10,padding:"12px",color:checkoutLoading?"#555":"#000",fontWeight:900,fontSize:14,cursor:checkoutLoading?"default":"pointer"},}
                , checkoutLoading?"Opening checkout...":"Start for $5.99/month →"
              )
            )
            ...
            , React.createElement('div', { style: {...card(),padding:"12px 14px",marginBottom:8,background:C.surface2},}
              ...
              , React.createElement('button', { onClick: ()=>openCheckout("business"), disabled: checkoutLoading, style: {marginTop:10,width:"100%",background:C.surface3,border:`0.5px solid ${C.border2}`,borderRadius:10,padding:"10px",color:"#AAA",fontWeight:700,fontSize:13,cursor:checkoutLoading?"default":"pointer"},}
                , checkoutLoading?"Opening checkout...":"Start for $17.99/month →"
              )
            )
